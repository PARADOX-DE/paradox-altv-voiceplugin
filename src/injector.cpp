...
DWORD CInjector::GetProcId(std::wstring targetProcessName)
{
	...
	do
	{
		if (std::wstring(entry.szExeFile) == targetProcessName) return entry.th32ProcessID;
	} while (Process32NextW(snap, &entry));

	return 0;
}

bool CInjector::ManualMap(HANDLE hProc, BYTE* pSrcData)
{
